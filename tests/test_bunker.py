"""First unit tests for BUNKER 2.0 core logic.

Run from the repository root with:  python3 -m pytest tests/ -v

These cover the pure/crypto pieces identified by the code-review council:
key derivation, vault encrypt/decrypt, database save/load round-trip and
...
def derived_key(vault):
    # Argon2id at 100 MB is intentionally slow; derive once per session.
...
class TestDatabaseSaveLoad:
    def test_round_trip(self, tmp_path, monkeypatch, derived_key):
    ...
    def test_no_temp_file_left_behind(self, tmp_path, monkeypatch, derived_key):
        monkeypatch.chdir(tmp_path)
        saveDatabase({"profiles": {}}, derived_key)
        leftovers = [f for f in os.listdir(".") if f.endswith(".tmp")]
        assert leftovers == []

    def test_failed_save_preserves_original(self, tmp_path, monkeypatch, derived_key):
        monkeypatch.chdir(tmp_path)
...
class TestAtomicWrite:
    def test_writes_content(self, tmp_path):
        ...
        _atomic_write_bytes(target, b"abc123")
        assert open(target, "rb").read() == b"abc123"
        assert not os.path.exists(target + ".tmp")

    def test_overwrites_existing(self, tmp_path):
        target = str(tmp_path / "out.bin")
        _atomic_write_bytes(target, b"old")
